...
def cross_row_dedup_status(user_id: int):
    ...
    return {"user_id": user_id, "n_seen": len(seen), "item_ids": seen[:50]}
...
def recommend(req: RecommendRequest):
    ...
    if experiment_group == "holdback_popularity":
        holdback_items = popularity_fallback(CATALOG, top_k=req.k)
        RETENTION.record_recommendation(uid, [i.get("item_id", 0) for i in holdback_items[:10]])
        return RecommendResponse(
            user_id=uid, k=req.k,
            items=[ScoredItem(item_id=i["item_id"], score=round(i.get("popularity", 1) / 200, 4),
                              als_score=0.0, ranker_score=0.0) for i in holdback_items],
...
def two_tower_status():
    ...
    return {
        "loaded":      _TWO_TOWER_RETRIEVER is not None,
...
def clip_search(
    ...
    if not getattr(CLIP_EMBEDDER, "available", False):
        ...
        return {"query": q, "results": results,
                "method": "text_embedding_fallback (CLIP not installed)",
