...
def ope_global(n_users: int = Query(50, ge=5, le=200)):
    ...
    import numpy as _np
    scores = []
    for u in _DEMO_USERS[:n_users]:
        uid = u["user_id"]
        recs = _build_recs(uid, k=10)
        ug  = _user_genres(uid)
        ugr = _user_ugr(uid)
        # Simulate propensity-weighted score
        sc = float(_np.mean([reward_score(r, ugr, ug, r) if _AI_MODULES_LOADED else 0.5 for r in recs[:10]]))
        scores.append(sc)
    if not scores:
...
def cold_start_recommend(user_id: int, genres: str = Query(""), k: int = Query(10)):
    ...
    ug  = _user_genres(uid)
    ugr = _user_ugr(uid)
    n_interactions = sum(1 for v in ugr.values() if v > 0) if ugr else 0
