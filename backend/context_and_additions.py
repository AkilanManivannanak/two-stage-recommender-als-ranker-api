...
def popularity_fallback(catalog: List[Dict], top_k: int = 30) -> List[Dict]:
    """
    Popularity-sorted item list for holdback users.
    Sorted by popularity_score descending, no personalisation.
    """
    sorted_items = sorted(
        catalog,
...
class CUPEDEstimator:
    ...
    def compute(self, alpha: float = 0.05) -> Dict:
        ...
        def welch_pvalue(a: List[float], b: List[float]) -> Tuple[float, float]:
            import math as _math   # explicit local import avoids UnboundLocalError
            na, nb      = len(a), len(b)
            mean_a      = sum(a) / na
            mean_b      = sum(b) / nb
            ...
            df       = df_num / max(df_denom, 1e-10)  # noqa: F841
